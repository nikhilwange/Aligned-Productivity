...
const StrategistView: React.FC<StrategistViewProps> = ({ recordings, userId }) => {
  ...
  const handleGenerateAnalysis = async () => {
    ...
    try {
      const result = await generateStrategicAnalysis(recordings);
      setAnalysis(result);
    } catch (err: any) {
  ...
  const completedMeetingsCount = recordings.filter(
      ...
      <header className="shrink-0 bg-[var(--surface-900)]/80 backdrop-blur-xl border-b border-white/[0.06] sticky top-0 z-40">
        <div className="h-16 flex items-center px-4 md:px-8 justify-between">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg glass text-xs font-semibold text-[var(--text-tertiary)]">
            ...
            {completedMeetingsCount} meetings analyzed
          </div>
        </div>
      </header>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {completedMeetingsCount === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="opacity-60 font-semibold text-lg mb-2">No meetings to analyze yet</h3>
            ...
            <div className="space-y-8">
              <div className="text-center py-12">
                ...
                <p className="text-[var(--text-secondary)] mb-8 max-w-lg mx-auto opacity-60">
                  Analyze {completedMeetingsCount} meetings to identify process gaps, strategic actions, and recurring issues across your workspace.
                </p>
                <button
                  ...
                  {analysis.strategicActions.map((action, idx) => (
                    <div key={idx} className="glass p-6 rounded-xl border border-white/10 hover:border-purple-500/30 transition-all group">
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="font-bold text-lg text-[var(--text-primary)]">{action.title}</h3>
              ...
              {activeSection === 'gaps' && (
                <div className="space-y-4">
                  {analysis.processGaps.map((gap, idx) => (
                    <div key={idx} className="glass p-6 rounded-xl border border-white/10 hover:border-purple-500/30 transition-all">
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="font-bold text-lg text-[var(--text-primary)]">{gap.title}</h3>
                        <div className="flex items-center gap-3">
              ...
              {activeSection === 'issues' && (
                <div className="space-y-4">
                  {analysis.issuePatterns.map((issue, idx) => (
                    <div key={idx} className="glass p-6 rounded-xl border border-white/10 hover:border-purple-500/30 transition-all">
                      <div className="flex items-start justify-between mb-3">
                        <h3 className="font-bold text-lg text-[var(--text-primary)]">{issue.issue}</h3>
                        <div className="flex items-center gap-3">

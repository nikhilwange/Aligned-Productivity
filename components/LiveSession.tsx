...
const LiveSession: React.FC<LiveSessionProps> = ({ onEndSession }) => {
  ...
  return (
    <div className="relative w-full h-full bg-[#020617] flex flex-col overflow-hidden text-slate-50">
      <div className="absolute inset-0 bg-gradient-to-b from-teal-500/10 via-transparent to-transparent pointer-events-none"></div>
      <div className="relative z-10 flex items-center justify-between p-8">
        <div className="flex items-center gap-3 px-4 py-2 bg-white/5 backdrop-blur-xl rounded-full border border-white/10">
          <div className={`w-2 h-2 rounded-full ${status === 'connected' ? 'bg-teal-400 animate-pulse' : 'bg-slate-600'}`}></div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative">
        <div className="relative group">
           <div className="absolute inset-0 bg-teal-500/20 blur-[120px] rounded-full animate-soft-pulse" style={{ transform: `scale(${1 + volume})` }}></div>
           <div className="relative w-48 h-48 md:w-64 md:h-64 rounded-full border border-white/10 bg-gradient-to-br from-white/10 to-transparent flex items-center justify-center backdrop-blur-md transition-transform duration-75" style={{ transform: `scale(${1 + volume * 0.2})` }}>
              ...
              <div className="absolute inset-4 border border-teal-500/20 rounded-full animate-[spin_10s_linear_infinite]"></div>
           </div>
        </div>
        <p className="mt-12 text-teal-400/60 font-bold tracking-[0.2em] text-[10px]">Speak naturally</p>
      </div>
         ...
         <div className="max-w-3xl mx-auto space-y-6 pb-4">
           {messages.map(m => (
             <div key={m.id} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} animate-in fade-in slide-in-from-bottom-2`}>
               <div className={`px-4 py-2.5 rounded-2xl text-sm ${m.role === 'user' ? 'bg-teal-500/10 text-teal-200 border border-teal-500/20' : 'bg-white/5 text-slate-300'}`}>{m.text}</div>

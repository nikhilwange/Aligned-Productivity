...
// inlines the _shared/cors.ts contents at the top so the entire function
// fits in a single file that can be pasted directly into the Supabase Edge
// Functions dashboard editor without missing-module errors. (This function
// does not use Portkey, so _shared/portkey.ts is NOT inlined.) The
// non-standalone index.ts is the canonical source — keep both in sync if
// either is edited.
//
